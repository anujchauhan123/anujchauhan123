...
class Employee(models.Model):
    ...
    def get_user_employee_details(self):
        ...
        leaves_to_approve = self.env['capwise.lead'].sudo()
        ...
        sanction_total = self.env['capwise.lead'].sudo().search_count([('sanction_date' ,'!=', False)])
        timesheet_count = self.env['capwise.lead'].sudo().search_count([('login_date_with_Bank_NBFC' ,'!=', False)])
        total_disbursed = self.env['capwise.lead'].sudo().search_count([('disb_Date' ,'!=', False)])
        job_applications = self.env['capwise.lead'].sudo().search_count([])

        bl_lead = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'bl')])
        hl_lead = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'hl')])
        pl_lead = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'pl')])
        lap_lead = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'lap')])

        channel_lead = self.env['crm.lead'].sudo().search_count([])
        on_board_lead = self.env['crm.lead'].sudo().search_count([('stage_id.is_won','=',True)])


        channel_unassigned = self.env['crm.lead'].sudo().search_count([('user_id','=',4)])


        total_lead_bl = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'bl')])
        total_logins_bl = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'bl'),('login_date_with_Bank_NBFC' ,'!=', False)])
        total_sanction_bl = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'bl'),('sanction_date' ,'!=', False)])
        total_disbursed_bl = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'bl'),('disb_Date' ,'!=', False)])


        total_lead_pl = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'pl')])
        total_logins_pl = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'pl'),('login_date_with_Bank_NBFC' ,'!=', False)])
        total_sanction_pl = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'pl'),('sanction_date' ,'!=', False)])
        total_disbursed_pl = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'pl'),('disb_Date' ,'!=', False)])


        total_lead_hl = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'hl')])
        total_logins_hl = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'hl'),('login_date_with_Bank_NBFC' ,'!=', False)])
        total_sanction_hl = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'hl'),('sanction_date' ,'!=', False)])
        total_disbursed_hl = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'hl'),('disb_Date' ,'!=', False)])


        total_lead_lap = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'lap')])
        total_logins_lap = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'lap'),('login_date_with_Bank_NBFC' ,'!=', False)])
        total_sanction_lap = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'lap'),('sanction_date' ,'!=', False)])
        total_disbursed_lap = self.env['capwise.lead'].sudo().search_count([('loan_type' ,'=', 'lap'),('disb_Date' ,'!=', False)])

        if employee:
            sql = """select * from capwise_lead where id =%s"""
            ...
            if employee:
                data = {
                    ...
                    'total_lead_bl' : total_lead_bl,
                    'total_logins_bl' : total_logins_bl,
                    'total_sanction_bl' : total_sanction_bl,
                    'total_disbursed_bl' : total_disbursed_bl,
                    'total_lead_pl' : total_lead_pl,
                    'total_logins_pl' : total_logins_pl,
                    'total_sanction_pl' : total_sanction_pl,
                    'total_disbursed_pl' : total_disbursed_pl,
                    'total_lead_hl' : total_lead_hl,
                    'total_logins_hl' : total_logins_hl,
                    'total_sanction_hl' : total_sanction_hl,
                    'total_disbursed_hl' : total_disbursed_hl,
                    'total_lead_lap' : total_lead_lap,
                    'total_logins_lap' : total_logins_lap,
                    'total_sanction_lap' : total_sanction_lap,
                    'total_disbursed_lap' : total_disbursed_lap,
                    'on_board_lead' : on_board_lead,
...
group by hr_employee.department_id,hr_department.name""")
    ...
    def get_department_leave(self):
        ...
        FROM  (select * from capwise_lead where state = 'validate') h
