...
class CRMLead(models.Model):
    ...
    def crm_quarter(self):
        ...
        self._cr.execute('''select COUNT(id) from capwise_lead WHERE capwise_lead.user_id = '%s'
        AND Extract(QUARTER FROM capwise_lead.date_deadline) = Extract(QUARTER FROM DATE(NOW()))
        AND Extract(Year FROM capwise_lead.date_deadline) = Extract(Year FROM DATE(NOW()))
        ''' % session_user_id)
        record = self._cr.dictfetchall()
        rec_ids = [item['count'] for item in record]
        crm_lead_value = rec_ids[0]
        ...
        self._cr.execute('''select COUNT(id) from capwise_lead  WHERE capwise_lead.user_id = %s
        AND capwise_lead.type = 'opportunity' AND Extract(QUARTER FROM capwise_lead.date_deadline
        ) = Extract(QUARTER FROM DATE(NOW())) AND Extract(Year FROM capwise_lead.date_deadline
        ) = Extract(Year FROM DATE(NOW( )))''' % session_user_id)
        opportunity_data = self._cr.dictfetchall()
        opportunity_data_value = [item['count'] for item in opportunity_data]
        opportunity_value = opportunity_data_value[0]

        self._cr.execute('''select SUM(capwise_lead.expected_revenue) from capwise_lead WHERE
        capwise_lead.user_id = %s and type='opportunity' and active='true' AND Extract(
        QUARTER FROM capwise_lead.date_deadline) = Extract(QUARTER FROM DATE(NOW())) AND
        Extract(Year FROM capwise_lead.date_deadline) = Extract(Year FROM DATE(NOW()))
        ''' % session_user_id)
        exp_revenue_data = self._cr.dictfetchall()
        exp_revenue_data_value = [item['sum'] for item in exp_revenue_data]
        exp_revenue_value = exp_revenue_data_value[0]
        ...
        self._cr.execute('''select SUM(capwise_lead.expected_revenue) from capwise_lead WHERE
        capwise_lead.user_id = %s and type='opportunity' and active='true' and stage_id=4
        AND Extract(QUARTER FROM capwise_lead.date_closed) = Extract(QUARTER FROM
        DATE(NOW())) AND Extract(Year FROM capwise_lead.date_closed) = Extract(Year
        FROM DATE(NOW()))''' % session_user_id)
        revenue_data = self._cr.dictfetchall()
        revenue_data_value = [item['sum'] for item in revenue_data]
        revenue_value = revenue_data_value[0]
        ...
        if revenue_value > 0:
            self._cr.execute('''select case when b.count_two = 0 then 0 else (
            CAST(a.count_one as float) / CAST(b.count_two as float))end as final_count
            from (select COUNT(id) as count_one from capwise_lead 
            WHERE capwise_lead.user_id = '%s' AND capwise_lead.active = True AND 
            capwise_lead.probability = 100 AND Extract(QUARTER FROM capwise_lead.date_deadline
            ) = Extract(QUARTER FROM DATE(NOW())) AND Extract(Year FROM capwise_lead.date_open
            ) = Extract(Year FROM DATE(NOW())))a,
            (select COUNT(id) as count_two from capwise_lead WHERE capwise_lead.user_id = '%s'
            AND capwise_lead.active = False AND capwise_lead.probability = 0 AND Extract(
            QUARTER FROM capwise_lead.date_deadline) = Extract(QUARTER FROM DATE(NOW()))
            AND Extract(Year FROM capwise_lead.date_deadline) = Extract(Year FROM DATE(NOW(
            ))))b''' % (session_user_id, session_user_id))
            ratio_value = [row[0] for row in self._cr.fetchall()]
            ratio_value = str(ratio_value)[1:-1]

        self._cr.execute('''SELECT active,count(active) FROM capwise_lead
        where type='opportunity' and active = true and probability = 100 and user_id=%s
        ...
        if crm_lead_value == 1:
            self._cr.execute('''SELECT id, date_conversion, create_date
            FROM capwise_lead WHERE date_conversion IS NOT NULL;''')
            data = self._cr.dictfetchall()
            for rec in data:
                date_close = rec['date_conversion']
                date_create = rec['create_date']
    ...
    def crm_month(self):
        ...
        self._cr.execute('''select COUNT(id) from capwise_lead WHERE capwise_lead.user_id = '%s'
        AND Extract(MONTH FROM capwise_lead.date_deadline) = Extract(MONTH FROM DATE(NOW()))
        AND Extract(Year FROM capwise_lead.date_deadline) = Extract(Year FROM DATE(NOW()))
        ''' % session_user_id)
        record = self._cr.dictfetchall()
        rec_ids = [item['count'] for item in record]
        crm_lead_value = rec_ids[0]
        print("crm_lead_value@@@@@@@@@@@@@@@@",crm_lead_value)

        self._cr.execute('''select COUNT(id) from capwise_lead WHERE capwise_lead.user_id = %s
        AND capwise_lead.type = 'opportunity' AND Extract(MONTH FROM capwise_lead.date_deadline
        ) = Extract(MONTH FROM DATE(NOW())) AND Extract(Year FROM capwise_lead.date_deadline
        ) = Extract(Year FROM DATE(NOW( )))''' % session_user_id)
        opportunity_data = self._cr.dictfetchall()
        opportunity_data_value = [item['count'] for item in opportunity_data]
        opportunity_value = opportunity_data_value[0]
        ...
        self._cr.execute('''select SUM(capwise_lead.expected_revenue) from capwise_lead WHERE
        capwise_lead.user_id = %s and type='opportunity' and active='true' AND Extract(
        MONTH FROM capwise_lead.date_deadline) = Extract(MONTH FROM DATE(NOW())) AND
        Extract(Year FROM capwise_lead.date_deadline) = Extract(Year FROM DATE(NOW()))
        ''' % session_user_id)
        exp_revenue_data = self._cr.dictfetchall()
        exp_revenue_data_value = [item['sum'] for item in exp_revenue_data]
        ...
        self._cr.execute('''select SUM(capwise_lead.expected_revenue) from capwise_lead WHERE
        capwise_lead.user_id = %s and type='opportunity' and active='true' and stage_id=4
        AND Extract(MONTH FROM capwise_lead.date_closed) = Extract(MONTH FROM DATE(NOW()))
        AND Extract(Year FROM capwise_lead.date_closed) = Extract(Year FROM DATE(NOW()))
        ''' % session_user_id)
        revenue_data = self._cr.dictfetchall()
        revenue_data_value = [item['sum'] for item in revenue_data]
        revenue_value = revenue_data_value[0]
        ...
        if revenue_value > 0:
            ...
            from (select COUNT(id) as count_one from capwise_lead
            WHERE capwise_lead.user_id = '%s' AND capwise_lead.active = True AND
            capwise_lead.probability = 100 AND Extract(MONTH FROM capwise_lead.date_deadline
            ) = Extract(MONTH FROM DATE(NOW())) AND Extract(Year FROM capwise_lead.date_open
            ) = Extract(Year FROM DATE(NOW())))a,
            (select COUNT(id) as count_two from capwise_lead WHERE capwise_lead.user_id = '%s'
            AND capwise_lead.active = False AND capwise_lead.probability = 0 AND Extract(
            MONTH FROM capwise_lead.date_deadline) = Extract(MONTH FROM DATE(NOW())) AND
            Extract(Year FROM capwise_lead.date_deadline) = Extract(Year FROM DATE(NOW())))b
            ''' % (session_user_id, session_user_id))
            ratio_value = [row[0] for row in self._cr.fetchall()]
            ratio_value = str(ratio_value)[1:-1]
    ...
    def crm_week(self):
        ...
        self._cr.execute('''select COUNT(id) from capwise_lead WHERE capwise_lead.user_id = '%s'
        AND Extract(MONTH FROM capwise_lead.date_deadline) = Extract(MONTH FROM DATE(NOW()))
        AND Extract(Week FROM capwise_lead.date_deadline) = Extract(Week FROM DATE(NOW())) AND
        Extract(Year FROM capwise_lead.date_deadline) = Extract(Year FROM DATE(NOW()))
        ''' % session_user_id)
        record = self._cr.dictfetchall()
        rec_ids = [item['count'] for item in record]
        ...
        self._cr.execute('''select COUNT(id) from capwise_lead WHERE capwise_lead.user_id = %s
        AND capwise_lead.type = 'opportunity' AND Extract(MONTH FROM capwise_lead.date_deadline
        ) = Extract(MONTH FROM DATE(NOW())) AND Extract(Week FROM capwise_lead.date_deadline
        ) = Extract(Week FROM DATE(NOW())) AND Extract(Year FROM capwise_lead.date_deadline
        ) = Extract(Year FROM DATE(NOW()))''' % session_user_id)
        opportunity_data = self._cr.dictfetchall()
        opportunity_data_value = [item['count'] for item in opportunity_data]
        opportunity_value = opportunity_data_value[0]
        ...
        capwise_lead.user_id = %s and type='opportunity' and active='true' AND Extract(
        MONTH FROM capwise_lead.date_deadline) = Extract(MONTH FROM DATE(NOW())) AND
        Extract(Week FROM capwise_lead.date_deadline) = Extract(Week FROM DATE(NOW()))
        AND Extract(Year FROM capwise_lead.date_deadline) = Extract(Year FROM DATE(NOW()))
        ''' % session_user_id)
        exp_revenue_data = self._cr.dictfetchall()
        exp_revenue_data_value = [item['sum'] for item in exp_revenue_data]
        ...
        capwise_lead.user_id = %s and type='opportunity' and active='true' and stage_id=4
        AND Extract(MONTH FROM capwise_lead.date_closed) = Extract(MONTH FROM DATE(NOW()))
        AND Extract(Week FROM capwise_lead.date_closed) = Extract(Week FROM DATE(NOW()))
        AND Extract(Year FROM capwise_lead.date_closed) = Extract(Year FROM DATE(NOW()))
        ''' % session_user_id)
        revenue_data = self._cr.dictfetchall()
        revenue_data_value = [item['sum'] for item in revenue_data]
        ...
        if revenue_value > 0:
            ...
            ) = Extract(Week FROM DATE(NOW())) AND Extract(Year FROM capwise_lead.date_open
            ) = Extract(Year FROM DATE(NOW())))a,
            (select COUNT(id) as count_two from capwise_lead WHERE capwise_lead.user_id = '%s'
            AND capwise_lead.active = False AND capwise_lead.probability = 0 AND Extract(MONTH
            FROM capwise_lead.date_deadline) = Extract(MONTH FROM DATE(NOW())) AND Extract(Week
            FROM capwise_lead.date_deadline) = Extract(Week FROM DATE(NOW())) AND Extract(Year FROM
            capwise_lead.date_deadline) = Extract(Year FROM DATE(NOW())))b
            ''' % (session_user_id, session_user_id))
            ratio_value = [row[0] for row in self._cr.fetchall()]
            ratio_value = str(ratio_value)[1:-1]
        ...
        or type='opportunity' and active = false and probability = 0 and user_id=%s
        AND Extract(MONTH FROM capwise_lead.date_deadline) = Extract(MONTH FROM DATE(NOW()))
        AND Extract(Week FROM capwise_lead.date_deadline) = Extract(Week FROM DATE(NOW()))
        AND Extract(Year FROM capwise_lead.date_deadline) = Extract(Year FROM DATE(NOW()))
        GROUP BY active''' % (session_user_id, session_user_id))
        record_opportunity = dict(self._cr.fetchall())
        opportunity_ratio_value = 0.0
